...
function interior_import_files() {
	...
	$demoImg = '<img src="'.plugins_url( 'screen-image.jpg', __FILE__ ) .'" alt="'.esc_attr__( 'Demo Preview Imgae', 'interior-companion' ).'" />';
	
  return array(
    array(
      'import_file_name'             => 'Interior Demo',
      'local_import_file'            => INTERIOR_COMPANION_DEMO_DIR_PATH .'interior-demo.xml',
      'local_import_widget_file'     => INTERIOR_COMPANION_DEMO_DIR_PATH .'interior-widgets-demo.wie',
      'import_customizer_file_url'   => plugins_url( 'interior-customizer.dat', __FILE__ ),
...
function interior_after_import_setup() {
	...
	interior_update_the_followed_post_page_status();

	// Add an option to check after import is done
	update_option( 'interior-import-data', true );
...
function interior_demo_import_custom_scripts(){
	...
	if( isset( $_GET['page'] ) && $_GET['page'] == 'interior-demo-import' ){
		// style
		wp_enqueue_style( 'interior-demo-import', plugins_url( 'css/demo-import.css', __FILE__ ), array(), '1.0', false );
	}
