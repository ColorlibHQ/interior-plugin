...
/**
 *
 * Interior elementor Interior video section widget.
 *
 * @since 1.0
 */
class Interior_Recipe_Video extends Widget_Base {

	public function get_name() {
		return 'interior-recipe-video';
	}

	public function get_title() {
		return __( 'Recipe Video', 'interior-companion' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Recipe Video content', 'interior-companion' ),
			]
        );
        $this->add_control(
            'sec_title',
            [
                ...
                'label_block' => true,
                'default'   => esc_html__( 'Recipe videos that never misses any portion', 'interior-companion' ),
            ]
        );
        $this->add_control(
            'sec_text',
            [
                ...
                'type' => Controls_Manager::TEXTAREA,
                'label_block' => true,
                'default'   => esc_html__( 'inappropriate behavior is often laughed off as “boys will be boys,” women face higher conduct standards especially in the workplace. That’s why it’s crucial that, as women.', 'interior-companion' ),
            ]
        );
        $this->add_control(
            'video_label_1',
            [
                'label' => esc_html__( 'Video Label 1', 'interior-companion' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default'   => esc_html__( 'Watch Video', 'interior-companion' ),
            ]
        );
        $this->add_control(
            'video_label_2',
            [
                'label' => esc_html__( 'Video Label 2', 'interior-companion' ),
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'default'   => esc_html__( 'You will love our execution', 'interior-companion' ),
            ]
        );
        $this->add_control(
            'video_url',
            [
                'label' => esc_html__( 'Video URL', 'interior-companion' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'description' => esc_html__( 'Best size is 205x230', 'interior-companion' ),
                'type' => Controls_Manager::MEDIA,
                'label_block' => true,
                'default'     => [
                    'url'   => Utils::get_placeholder_image_src(),
    ...
    $left_img = !empty( $settings['left_img']['id'] ) ? wp_get_attachment_image( $settings['left_img']['id'], 'interior_video_small_thumb_205x230', '', array( 'alt' => $sec_title ) ) : '';
    $bottom_img = !empty( $settings['bottom_img']['id'] ) ? wp_get_attachment_image( $settings['bottom_img']['id'], 'interior_video_small_thumb_205x230', '', array( 'alt' => $sec_title ) ) : '';
    ?>
